...
[CustomEditor(typeof(MonaSuperRunPowerup))]
public class MonaSuperRunPowerupEditor : Editor
{
    ...
    {
        MonaSuperRunPowerup MonaSuperRunPowerup = (MonaSuperRunPowerup)target;
        GUILayout.BeginVertical(backgroundStyle);

        if (!string.IsNullOrEmpty(MonaSuperRunPowerup.imagePath))
        {
            Texture2D myImage = AssetDatabase.LoadAssetAtPath<Texture2D>(MonaSuperRunPowerup.imagePath);
            if (myImage != null)
            {
            ...
            {
                EditorGUILayout.LabelField("Image not found at path:", MonaSuperRunPowerup.imagePath);
            }
        }
        ...
        EditorGUILayout.LabelField(MonaSuperRunPowerup.ModuleName, textStyle, GUILayout.Height(textSize + 4));
        EditorGUILayout.LabelField(MonaSuperRunPowerup.ModuleCreator, textStyle, GUILayout.Height(textSize + 4));
        ...
        if (GUILayout.Button(MonaSuperRunPowerup.ButtonTitle2, hyperlinkTextStyle, GUILayout.Height(textSize + buttonSpace)))
        {
            Application.OpenURL(MonaSuperRunPowerup.Link2);
        }

        GUILayout.Space(1);

        if (GUILayout.Button(MonaSuperRunPowerup.ButtonTitle3, hyperlinkTextStyle, GUILayout.Height(textSize + buttonSpace)))
        {
            Application.OpenURL(MonaSuperRunPowerup.Link3);
        }
