...
[CustomEditor(typeof(MonaPendulumTrap))]
public class MonaPendulumTrapEditor : Editor
{
    ...
    {
        MonaPendulumTrap MonaPendulumTrap = (MonaPendulumTrap)target;
        GUILayout.BeginVertical(backgroundStyle);

        if (!string.IsNullOrEmpty(MonaPendulumTrap.imagePath))
        {
            Texture2D myImage = AssetDatabase.LoadAssetAtPath<Texture2D>(MonaPendulumTrap.imagePath);
            if (myImage != null)
            {
            ...
            {
                EditorGUILayout.LabelField("Image not found at path:", MonaPendulumTrap.imagePath);
            }
        }
        ...
        EditorGUILayout.LabelField(MonaPendulumTrap.ModuleName, textStyle, GUILayout.Height(textSize + 4));
        EditorGUILayout.LabelField(MonaPendulumTrap.ModuleCreator, textStyle, GUILayout.Height(textSize + 4));
        ...
        if (GUILayout.Button(MonaPendulumTrap.ButtonTitle2, hyperlinkTextStyle, GUILayout.Height(textSize + buttonSpace)))
        {
            Application.OpenURL(MonaPendulumTrap.Link2);
        }

        GUILayout.Space(1);

        if (GUILayout.Button(MonaPendulumTrap.ButtonTitle3, hyperlinkTextStyle, GUILayout.Height(textSize + buttonSpace)))
        {
            Application.OpenURL(MonaPendulumTrap.Link3);
        }
