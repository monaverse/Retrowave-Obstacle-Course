...
[CustomEditor(typeof(MonaMovingPlatform))]
public class MonaMovingPlatformEditor : Editor
{
    ...
    {
        MonaMovingPlatform MonaMovingPlatform = (MonaMovingPlatform)target;
        GUILayout.BeginVertical(backgroundStyle);

        if (!string.IsNullOrEmpty(MonaMovingPlatform.imagePath))
        {
            Texture2D myImage = AssetDatabase.LoadAssetAtPath<Texture2D>(MonaMovingPlatform.imagePath);
            if (myImage != null)
            {
            ...
            {
                EditorGUILayout.LabelField("Image not found at path:", MonaMovingPlatform.imagePath);
            }
        }
        ...
        EditorGUILayout.LabelField(MonaMovingPlatform.ModuleName, textStyle, GUILayout.Height(textSize + 4));
        EditorGUILayout.LabelField(MonaMovingPlatform.ModuleCreator, textStyle, GUILayout.Height(textSize + 4));
        ...
        if (GUILayout.Button(MonaMovingPlatform.ButtonTitle2, hyperlinkTextStyle, GUILayout.Height(textSize + buttonSpace)))
        {
            Application.OpenURL(MonaMovingPlatform.Link2);
        }

        GUILayout.Space(1);

        if (GUILayout.Button(MonaMovingPlatform.ButtonTitle3, hyperlinkTextStyle, GUILayout.Height(textSize + buttonSpace)))
        {
            Application.OpenURL(MonaMovingPlatform.Link3);
        }
