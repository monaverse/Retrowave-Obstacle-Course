...
[CustomEditor(typeof(MonaCutScene))]
public class MonaCutSceneEditor : Editor
{
    ...
    {
        MonaCutScene MonaCutScene = (MonaCutScene)target;
        GUILayout.BeginVertical(backgroundStyle);

        if (!string.IsNullOrEmpty(MonaCutScene.imagePath))
        {
            Texture2D myImage = AssetDatabase.LoadAssetAtPath<Texture2D>(MonaCutScene.imagePath);
            if (myImage != null)
            {
            ...
            {
                EditorGUILayout.LabelField("Image not found at path:", MonaCutScene.imagePath);
            }
        }
        ...
        EditorGUILayout.LabelField(MonaCutScene.ModuleName, textStyle, GUILayout.Height(textSize + 4));
        EditorGUILayout.LabelField(MonaCutScene.ModuleCreator, textStyle, GUILayout.Height(textSize + 4));
        ...
        if (GUILayout.Button(MonaCutScene.ButtonTitle2, hyperlinkTextStyle, GUILayout.Height(textSize + buttonSpace)))
        {
            Application.OpenURL(MonaCutScene.Link2);
        }

        GUILayout.Space(1);

        if (GUILayout.Button(MonaCutScene.ButtonTitle3, hyperlinkTextStyle, GUILayout.Height(textSize + buttonSpace)))
        {
            Application.OpenURL(MonaCutScene.Link3);
        }
