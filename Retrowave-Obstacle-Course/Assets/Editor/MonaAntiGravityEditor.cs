...
[CustomEditor(typeof(MonaAntiGravity))]
public class MonaAntiGravityEditor : Editor
{
    ...
    {
        MonaAntiGravity MonaAntiGravity = (MonaAntiGravity)target;
        GUILayout.BeginVertical(backgroundStyle);

        if (!string.IsNullOrEmpty(MonaAntiGravity.imagePath))
        {
            Texture2D myImage = AssetDatabase.LoadAssetAtPath<Texture2D>(MonaAntiGravity.imagePath);
            if (myImage != null)
            {
            ...
            {
                EditorGUILayout.LabelField("Image not found at path:", MonaAntiGravity.imagePath);
            }
        }
        ...
        EditorGUILayout.LabelField(MonaAntiGravity.ModuleName, textStyle, GUILayout.Height(textSize + 4));
        EditorGUILayout.LabelField(MonaAntiGravity.ModuleCreator, textStyle, GUILayout.Height(textSize + 4));
        ...
        if (GUILayout.Button(MonaAntiGravity.ButtonTitle2, hyperlinkTextStyle, GUILayout.Height(textSize + buttonSpace)))
        {
            Application.OpenURL(MonaAntiGravity.Link2);
        }

        GUILayout.Space(1);

        if (GUILayout.Button(MonaAntiGravity.ButtonTitle3, hyperlinkTextStyle, GUILayout.Height(textSize + buttonSpace)))
        {
            Application.OpenURL(MonaAntiGravity.Link3);
        }
