...
[CustomEditor(typeof(MonaStartGate))]
public class MonaStartGateEditor : Editor
{
    ...
    {
        MonaStartGate MonaStartGate = (MonaStartGate)target;
        GUILayout.BeginVertical(backgroundStyle);

        if (!string.IsNullOrEmpty(MonaStartGate.imagePath))
        {
            Texture2D myImage = AssetDatabase.LoadAssetAtPath<Texture2D>(MonaStartGate.imagePath);
            if (myImage != null)
            {
            ...
            {
                EditorGUILayout.LabelField("Image not found at path:", MonaStartGate.imagePath);
            }
        }
        ...
        EditorGUILayout.LabelField(MonaStartGate.ModuleName, textStyle, GUILayout.Height(textSize + 4));
        EditorGUILayout.LabelField(MonaStartGate.ModuleCreator, textStyle, GUILayout.Height(textSize + 4));
        ...
        if (GUILayout.Button(MonaStartGate.ButtonTitle2, hyperlinkTextStyle, GUILayout.Height(textSize + buttonSpace)))
        {
            Application.OpenURL(MonaStartGate.Link2);
        }

        GUILayout.Space(1);

        if (GUILayout.Button(MonaStartGate.ButtonTitle3, hyperlinkTextStyle, GUILayout.Height(textSize + buttonSpace)))
        {
            Application.OpenURL(MonaStartGate.Link3);
        }
